...
export async function POST(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (quest.status === "DONE") return NextResponse.json({ message: "DONEは受注できません" }, { status: 400 });

  await auth.supabase.from("quest_acceptances").upsert({ quest_id: id, user_id: auth.user.id });

  if (quest.status === "OPEN") {
    await auth.supabase.from("quests").update({ status: "DOING" }).eq("id", id);
  }
